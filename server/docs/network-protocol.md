...
| `pick_trait_choice` | `index` -1..7 | (revision 5) answer the pending trait choice in `state.trait_choice`: an option index; `-1` declines a **prototype** (Najmi: the star dust stays banked; a Hexagon **module** must be picked: `InvalidSlot`). `AlreadyPicked` when nothing is pending. Planning, Combat, Resolution; whatever is still open when the next Combat begins is settled for the player (a module: the first option). (revision 6) A Najmi cash-out is NOT settled: it stays open until picked or replaced by a bigger one |
...
| `state` | whenever anything private changed | `player_id`, `alive`, `health`, `gold`, `level`, `xp`, `xp_to_next`, `streak`, `shop` (champion ids, 0 = empty), `bench` (9 entries: a unit or `null`), `board` (units), `item_bag` (item ids), `gifts` (Mother Nature: the offers still open, each `{index, gift, name, kind: gold\|xp\|heal\|item\|unit, amount \| item \| champion + cost}`; empty once picked and outside her phase), `gift_settled`, (revision 5) `traits` (`star_dust`, `trait_gold`, `takedown_counter`, `unit_granted`, `modules`: chosen Hexagon module ids; revision 6: `unlocked`: unlockable champions this player's shop may offer), `trait_choice` (as in `trait_choice`; kind `none` when nothing is pending) |
| `trait_choice` | (revision 5) a trait asks / the choice was made | `event` `offered` (`choice`) or `resolved` (`choice`, `index`: -1 = declined, `automatic`: the phase ran out). A `choice` is `{kind: none\|module\|prototype, trait, tier, options: [module ids \| item ids], bonus_gold, bonus_items (revision 6: a Najmi cash-out's extra items), can_decline}`; a prototype's `tier` is the cash-out = hundreds of star dust (1..6) |
| `champion_unlocked` | (revision 6) the player unlocked an unlockable champion (Hexa: 7 Hexagon star levels at level 8) | `champion`: it can now appear in their shop |
| `trait_rewards` | (revision 5) after a combat, when the player's traits paid anything | `xp` (Selini Enlightenment), `gold` (Selini Prosperity takedowns, Hexagon's Mining Drill), `star_dust` (gained), `star_dust_total` (the Najmi bank), `unit` (a champion given to the bench: the Rift Herald; 0 = none) |
